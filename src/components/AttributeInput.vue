    ...
    >
      ...
      >
        <q-field v-if="value.dataType.type === 'Int'"
                 :hint="getHint(value)"
        >
          <q-input class="self-center full-width no-outline" v-model="value.input" @change="check(key, value)" autofocus :readonly="!edit"/>
        </q-field>
        ...
        <q-field v-else-if="value.dataType.type === 'Bool'"
                 :hint="getHint(value)"
        >
          <q-checkbox toggle-indeterminate v-model="value.input" @input="check(key, value)" :disable="!edit"/>
        </q-field>
        <q-field v-else-if="value.dataType.type === 'Enum'"
                 :hint="getHint(value)"
        >
          <q-select class="self-center full-width no-outline" v-model="value.input" :options="getOptions(value.dataType.enumType)" @change="check(key, value)"  :readonly="!edit"/>
        </q-field>
        ...
        <q-stepper-navigation content>
          ...
          <div class="q-gutter-xs q-my-xs">
            ...
            <q-btn color="primary" label="添加" @click="addArray(key)" v-if="type === 'Array' && edit" ></q-btn>
            <q-btn color="primary" label="删除" @click="deleteArray(key)" v-if="showDelete(type, key) && edit" ></q-btn>
          </div>
  ...
  methods: {
    ...
    convert (inputValue, info) {
      ...
      for (var key in this.attDefinition) {
        ...
        var result
        if (type === 'Array') {
          result = this.convertArray(definition.dataType.dataType, value, info)
        } else if (type === 'Struct') {
          result = this.convertStruct(definition.dataType.attTypes, value, info)
        } else if (type === 'Choice') {
          result = this.convertChoice(definition.dataType.attTypes, value, info)
        } else {
          result = this.convertStr(definition.dataType, value, info)
        }
        requestInfo[key] = result
      }
      return requestInfo
    },
    convertChoice (definition, value, info) {
      ...
      if (info) {
        ...
        let subValue = this.convertStruct(choiceDef[choiceKey].dataType.attTypes, value[choiceKey])
        let sub = {}
        sub[choiceKey] = subValue
        return {
          type: 'Choice',
          value: sub,
          string: null
    ...
    convertStruct (definition, value, info) {
      var structInfo = {}
    ...
    convertArray (definition, value, info) {
      var arrayInfo = []
    ...
    convertStr (definition, value, info) {
      if (info) {
        return { value: value }
      } else {
        let v
